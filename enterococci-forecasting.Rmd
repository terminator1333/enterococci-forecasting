...
required_packages <- c("dplyr", "ggplot2", "tidyr", "corrplot", "car", "broom", "scales", "viridis")
...
water_quality_multi <- water_quality_multi %>%
  mutate(
    ...
    season = as.factor(season),
    month = as.factor(month),
    risk_category = as.factor(risk_category)  # Multiclass target
  )

#fitting multinomial logistic regression
multi_logit_model <- multinom(
  risk_category ~ . - enterococci_cfu_100ml - log_enterococci,
  data = water_quality_multi
)
...
selected_features <- c(
  ...
  "water_temperature_c", "conductivity_ms_cm",
  "latitude", "longitude",
  "max_temp_C_scaled", "water_temp_scaled", "conductivity_scaled",
  "precipitation_mm", "precip_3day_avg",
  "temp_gradient", "temp_diff", "temp_conductivity",
...
thresholds <- c(Bad = 0.35, Poor = 0.6, Fair = 0.75, Safe = 1.5)
classes <- c("Bad", "Poor", "Fair", "Safe")
...
for (cls in classes) {
  ...
  print(importance_bin %>% arrange(desc(Gain)) %>% slice_head(n = 5))
}
...
thresholds <- c(Bad = 0.35, Poor = 0.95, Fair = 0.5, Safe = 1.6)
classes <- c("Bad", "Poor", "Fair", "Safe")
